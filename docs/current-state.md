...
| `tools/webtest/server.py` + `upload_support.py` | 4 screens; **Upload screen functional** (not pipeline-wired) | Flask serves 4 screens via `templates/base.html` (top nav, single-column per design.md §3): `/`→Upload, `/processing`, `/results`, `/settings`. **Upload screen is live** (design.md §2.1): file-picker **and** drag/drop upload with two-stage validation (extension allowlist → real OpenCV decode of frame 0), first-frame extraction shown on a `<canvas>`, click-drag rectangular static zones with per-zone delete, Blur/fake_data mode toggle. Zones are converted **display→original-video-pixel coords server-side** in `/zone` (D19) and stored as `[x,y,w,h]` lists directly consumable by `ZoneManager.add_zone()`; per-upload state (`video_path`, `mode`, `zones`) held in a process-local `_STATE` dict for the later pipeline-wiring task. `/process` returns a controlled **HTTP 501 placeholder** — `process_video` is deliberately **not** called yet. Pure validation + coordinate math extracted to `upload_support.py` (no Flask import). Processing/Results/Settings still static placeholders (later tasks). No new deps. |
