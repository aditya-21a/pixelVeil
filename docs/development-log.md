...
## 2026-08-11 — Standardized face pixelation divisor to 17, upload screen face settings auto-save
- Changed: core/redactor.py (`redact_face`), tools/webtest/templates/upload.html (layout updated, pixelate intensity select removed, added standard 17 indicator, fakeDataNote explanation added), tools/webtest/static/js/app.js (upload face IIFE updated, unconditional mode sync on upload, mode sent on `/process` POST), tools/webtest/server.py (`/process` overrides state mode from body), tools/webtest/job.py (`ProcessingJob._run` summary frames aggregation), tests/test_webtest_dual_output.py (test assertion updated), docs/current-state.md, docs/TASKS.md, docs/development-log.md.
- Why: User requested removing all low/medium/high options for pixelation and standardizing to divisor 17. Also ensured face anonymization settings are on the upload page, they auto-save on change, and redaction mode selection does not cause stale state or duplicate-pass (240 frame) runs. Fixed multi-pass summary aggregation so `frames_processed` is summed dynamically across passes (e.g. 240 frames for a 120-frame video in fake_data mode) without doubling unique detection counts.
- Result: Face pixelation standard factor divisor of 17 implemented. Upload screen face anonymization settings auto-save. Stale mode sync issues resolved unconditionally. Multi-pass frames processed aggregation implemented. Test suite passes fully (294 passed).
- Commit: uncommitted
