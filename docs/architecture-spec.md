...
graph TD
    ...
    SD --> TR[TargetedROIDetector]
    TR --> CV
    
    KT --> GR[GapResolver Offline]
    GR --> PSL[PrivacySafetyLayer]
...
## 2. FullFrameDetector (SCRFD-500M)
...
- **Parameters**:
  ...
  - `model`: `det_500m.onnx` [A: PixelVeil-validated]
- **Dependencies**: ONNX Runtime (CUDA/CPU).
- **Computational cost**: ~15ms CUDA, ~34ms CPU [A: PixelVeil benchmarked].
- **Why It Exists**: Primary detection mechanism for identifying faces.
- **Research Evidence**: Phase 1 — SCRFD-0.5GF achieves 68.5% WIDER FACE Hard AP.
...
- **Algorithm**:
  ```python
  strip_width = max(64, int(frame_width * 0.10))  # [C: Reasonable starting value]

  boundary_rois = [
      frame[0:strip_width, :],                 # top strip
      frame[-strip_width:, :],                 # bottom strip
      frame[:, 0:strip_width],                 # left strip
      frame[:, -strip_width:]                  # right strip
  ]
  ...
  ```
- **Conditions**: Operates on every frame or sampled frames if compute constrained.
- **Failure behavior**: If boundary scanning is too slow, reduce to 2 strips (left+right only) or scan every 2nd frame.
- **Parameters**:
  - `strip_width`: max(64, 10% of frame dimension) [C: Reasonable starting value]
  - `confidence_threshold`: 0.3 for boundary (lower than full-frame 0.5) [C: Reasonable starting value]
- **Dependencies**: SCRFD detector.
- **Computational cost**: 4 small SCRFD inferences. Each strip is much smaller than full frame, so ~3-5ms each on CUDA. Total ~12-20ms.
- **Why It Exists**: This is the PRIMARY solution to the brand-new face problem. Without a prior track, there is no signal to trigger recovery. Boundary scanning proactively checks likely entry points.
- **Research Evidence**: Phase 1 — zero-padding boundary artifacts cause catastrophic edge detection failures. Phase 6 — no reliable way to detect never-seen faces retroactively.

---

## 4. CandidateValidator

- **Purpose**: Filter false positive detections without destroying recall on small/partial faces.
- **Inputs**: Detection `(x, y, w, h, confidence)`, optional tracker state.
- **Outputs**: Boolean (valid/invalid).
- **State**: None (Stateless).
- **Algorithm**:
  ```python
  # Stage 1: Geometric validation [O(1)]
  IF width < 20 OR height < 20: REJECT  # minimum face size
  IF aspect_ratio < 0.5 OR aspect_ratio > 2.0: REJECT  # impossible face shape
  ...
  IF has_matching_track:
      ...
      IF mahalanobis_dist > 9.48: REJECT  # chi-squared 95%, 4 DOF

  ACCEPT  # Privacy-biased: accept by default
  ```
- **Conditions**: Always runs on new detections.
- **Failure behavior**: If valid faces are rejected, lower thresholds.
- **Parameters**:
  - `min_face_size`: 20x20 [B: Literature]
...
- **Why It Exists**: Prunes noise from lower confidence thresholds while preserving privacy recall.
- **Research Evidence**: Phase 3 — Cascaded validation prunes ~80% noise at O(1). Privacy recall requires accepting by default.
...
- **State**: Track states: TENTATIVE, CONFIRMED, COASTING, EXPIRED. Track vector: `[cx, cy, w, h, vx, vy, vw, vh]` (8-dimensional). Covariance matrix `P`.
- **Algorithm**:
  ```python
  FOR each track:
  ...
  FOR matched tracks: kalman_update(track, detection)
...
- **Algorithm**:
  ...
  **Case A: Mid-Track Gap (✓ ✓ ✓ ? ? ✓ ✓ ✓)**
  ...
  FOR each gap in track:
      ...
      IF gap_length <= 5:  # Short gap
          ...
          cycle_error = distance(forward, backward)
          
          IF cycle_error < 3.0:  # [C: Reasonable starting value]
              fill_gap(interpolated, uncertainty='hourglass')
          ELSE:
              # Fall back to protective redaction
              fill_gap_with_protection(start_bbox, expansion=1.3)
  ...
  **Case B: Track-Start Gap (? ? ? ✓ ✓ ✓)**
  ...
  FOR each track with first_frame > 0:
      ...
      max_backward = min(20, search_start)  # [B: Literature]
      ...
      FOR frame_offset in range(1, max_backward + 1):
          ...
          IF search_roi.area > 1.5 * track.bbox.area:  # [C: Reasonable starting value]
          ...
          IF detections:
              ...
              cycle_error = forward_backward_check(detections, track)
              IF cycle_error <= 1.0:  # [B: Literature, strict]
                  extend_track_backward(track, detections, target_frame)
              ELSE:
                  # Detection found but inconsistent - apply protective redaction
                  apply_protective_redaction(predicted_bbox, target_frame)
                  BREAK
          ELSE:
              # No detection found - apply protective redaction for this frame
              apply_protective_redaction(predicted_bbox, target_frame)
              # Continue searching - might find face in earlier frame
  ```
...
- **Parameters**:
  - `max_backward_search`: 20 frames [B: Literature]
  - `cycle_consistency_threshold`: 1.0px for backward, 3.0px for interpolation [B/C]
  - `max_interpolation_gap`: 15 frames [C: Reasonable starting value]
  - `uncertainty_expansion_cap`: 1.5x [C: Reasonable starting value]
...
- **Research Evidence**: Phase 5 — GSI superior to RTS/linear. Phase 6 — 20-frame ceiling, cycle consistency.
...
- **Algorithm**:
  ...
  ELSE IF track.state == COASTING:
      # Expand based on tracker covariance
      sigma_x = sqrt(track.P[0,0])  # position uncertainty
      sigma_y = sqrt(track.P[1,1])
      uncertainty_expansion = Z * max(sigma_x, sigma_y)  # Z=2.58 for 99%
      uncertainty_expansion = min(uncertainty_expansion, 0.5 * max(expanded.w, expanded.h))  # cap at 1.5x
  ...
  noise_sigma = 10  # [C: Reasonable starting value]
  feather_width = max(16, int(0.15 * face_width))  # [B: Literature, >= 1 macroblock]

  return final_region, block_size, noise_sigma, feather_width
  ```
- **Conditions**: Applied before rendering to output.
- **Failure behavior**: If expansion is too aggressive (covering too much background), reduce `Z`.
- **Parameters**:
  - `Z` (uncertainty multiplier): 2.58 (99% confidence) [C: Reasonable starting value]
  - `max_expansion_ratio`: 1.5x [C: Reasonable starting value]
  - `base_pixelation_scalar`: 0.08 * W_head [B: Literature]
  - `noise_sigma`: 10 [C: Reasonable starting value]
  - `min_feather`: 16px [B: Literature]
...
- **Research Evidence**: Phase 8 — adaptive pixelation, noise overlay disrupts reconstruction, feathering prevents H.264 leakage.
